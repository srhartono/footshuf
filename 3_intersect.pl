...
foreach my $peak (sort keys %{$data->{perPeak}}) {
	...
	$data->{perPeak}{$peak}{origmeanperc} = asperc($data->{perPeak}{$peak}{orig} / $totalFootLoopPeaks);
	$data->{perPeak}{$peak}{shufmeanperc} = asperc(mean($data->{perPeak}{$peak}{shuf})/$totalFootLoopPeaks);
	$data->{perPeak}{$peak}{shufsdperc}   = asperc(sdev($data->{perPeak}{$peak}{shuf})/$totalFootLoopPeaks);
	...
	$data->{perPeak}{$peak}{fold}     = myformat(($data->{perPeak}{$peak}{origmeanperc} + 1)/($data->{perPeak}{$peak}{shufmeanperc} + 1),1000);
...
sub parse_intersectFile {
	...
	while (my $line = <$in2>) {
		...
		my $peak2 = $name2;
		$data->{orig}{$peak1} ++ if $count1 eq 0;
		$data->{shuf}{$peak1}[$count1-1] ++ if $count1 ne 0;
